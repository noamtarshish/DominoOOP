...
class NaivePlayer(Player):
    ...
    def play(self, board):
        """
        Function that turn on the game strategy for the NaivePlayer. The NaivePlayer add the first domino tilt that
        he can to the board. First, he try to add the domino tilt from the right side of the board and if he fail he
        try to add the domino tilt to the left side of the board.
        :param board: the board for the game
        :return: nothing
        """
        for i in range(len(self.hand)):
            if board.add_right(self.hand[i]) is True:
                self.hand.remove_domino(self.hand[i])
                return True

            if board.add_left(self.hand[i]) is True:
                self.hand.remove_domino(self.hand[i])
                return True
